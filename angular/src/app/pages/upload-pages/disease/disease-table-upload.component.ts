...
@Component({
  selector: 'ngx-disease-table-upload',
  templateUrl: './disease-table-upload.component.html',
  styleUrls: ['./disease-table-upload.component.scss', '../upload-pages.component.scss'],
})
export class DiseaseTableUploadComponent implements OnInit {
  data: any[];
  filename: string;
  package_name: string;
  ...
  uploadFile(): void {
    ...
    setTimeout(() => {
      this.backupHistoryRecall(this.user.getUsername(), 'disease', this.package_name);
    }, 1300);
    setTimeout(() => {
      this.loadingMessage = '';
  ...
  postSendData(user, package_name, data) {
    ...
    this.http.sendPostDiseaseTableData(body).then(
      (resp: UploadResponse) => {
        // tslint:disable-next-line: no-console
